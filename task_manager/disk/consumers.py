...
class DiskMonitorConsumer(AsyncWebsocketConsumer):
    ...
    async def _stream_loop(self):
        try:
            while True:
                data = await asyncio.to_thread(services.get_disk_status)
...
class DiskHistoryConsumer(AsyncWebsocketConsumer):
    channel_layer_alias = "task_manager"
    ...
    async def _stream_loop(self):
        try:
            while True:
                data = await asyncio.to_thread(services.get_disk_history)
                await self.send(text_data=json.dumps({
                    "type": "disk_history",
                    "data": data,
                }))
...
class DiskBreakdownConsumer(AsyncWebsocketConsumer):
    channel_layer_alias = "task_manager"
    ...
    async def _stream_loop(self):
        try:
            while True:
                data = await asyncio.to_thread(services.get_disk_breakdown)
                await self.send(text_data=json.dumps({
                    "type": "disk_breakdown",
                    "data": data,
                }))
...
class DiskProcessesConsumer(AsyncWebsocketConsumer):
    channel_layer_alias = "task_manager"
    ...
    async def _stream_loop(self):
        try:
            while True:
                data = await asyncio.to_thread(services.get_disk_processes, limit=self.limit)
                await self.send(text_data=json.dumps({
                    "type": "disk_processes",
                    "data": data,
                }))
